...
_start:
  ...
  mov	esi, 0		; initialize our checkBuzz boolean variable
  mov	edi, 0		; initialize our checkFizz boolean variable
...
.checkFizz:
  mov	edx, 0		; clear the edx register - this will hold the remainder after division
...
.checkBuzz:
  ...
  mov	eax, ecx	; move the value of our counter into eax for division
  mov	ebx, 5		; move our number to divide by into eax (in this case the value is 5)
  div	ebx		; divide eax by ebx
  mov	esi, edx	; move our remainder into esi (our checkBuzz boolean variable)
  cmp	esi, 0		; compare if the remainder is zero (meaning the counter divides by 5)
  jne	.checkInt	; if the remainder is not equal to zero jump to the local label checkInt
...
.continue:
  mov	eax, 0Ah	; move an ascii linefeed into eax
  ...
  call	sprint		; call our string printing function to print a linefeed
